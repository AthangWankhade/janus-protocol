...
export const NoteService = {
  ...
  async createNote(
    ...
    content: string,
    masterKey: Buffer,
    imageUris: string[] = [], // Changed to array
    audioUri?: string,
    docFiles: NoteFile[] = [] // Changed to array
  ) {
    let encryptedAudioPath = null;

    if (audioUri) {
      encryptedAudioPath = await StorageService.saveEncryptedFile(
        audioUri,
        masterKey,
        "audio_enc"
      );
    ...
    await database.write(async () => {
      const newNote = await notesCollection.create((note: any) => {
        ...
        note.audioUri = encryptedAudioPath;
        note.isPinned = false;
        note.createdAt = new Date();

        // Legacy fields (optional: keep null or set first item)
        note.imageUri = null;
        note.fileUri = null;
        note.fileName = null;
        note.fileMime = null;
      ...
      // <--- TRIGGER SYNC PUSH (Simplified for now, syncs core note)
      syncService.sendChange("CREATE", "notes", {
        id: newNote.id,
        title: newNote.title,
        content: newNote.content,
        audioUri: newNote.audioUri,
      });
    });
  },
    ...
    const decryptedNotes = await Promise.all(
      allNotes.map(async (note: any) => {
        try {
          ...
          if (note.audioUri) {
            playAudio = await StorageService.loadEncryptedFile(
              note.audioUri,
              masterKey,
              true,
              "m4a"
